...
public class StudentEx01 {
	...
	public static void main(String[] args) {
		 ...
		 * - 기본 생성자과 학년, 반, 번호, 이름을 이용한 생성자를 추가
		...
		int menu;
		int studentCount = 0;
		int index; // 번지를 저장하기 위한 변수
		int kor, eng, math;
		Student tmp;
		
		do {
			printMenu(); // 메뉴를 출력하는 메소드
			menu = scan.nextInt();
			// runMenu(menu); // 메뉴에 맞는 기능을 실행하는 메소드
			switch(menu){
			case 1: 
				// 만든 메서드 호출
			...
			case 4: 
				System.err.println(menu + "번, 프로그램을 종료합니다.");
				break;
				
			default: 
				System.err.println("잘못된 메뉴 값을 입력하셨습니다. 1 ~ 4 사이의 값으로 입력해주세요.");
			}
	...
	public static void runMenu(int menu) {
		switch(menu){
		case 1:
			System.out.println(menu + "번, 학생 등록 입니다.");
			break;
			
		case 2: 
			System.out.println(menu + "번, 성적 수정 입니다.");
			break;
		
		case 3: 
			System.out.println(menu + "번, 성적 확인 입니다.");
			break;
	 ...
	 * @param std 검색할 학생 정보
	 * @return 검색한 학생 정보의 위치(번지)로 없으면 -1로 반환
	 */
	...
	/**
	 * 기능 : 학생 정보를 입력 받아 학생 리스트에 추가하고 등록된 학생 숫자를 알려주는 메소드
	 * @param list 학생 리스트
	 * @param scan 학생 정보를 입력 받기 위한 Scanner
	 * @param studentCount 현재 학생 숫자
	 * @return 등록된 학생 숫자
	 */
	public static int insertStudent(Student [] list, Scanner scan, int studentCount) {
		// 리스트에 학생이 꽉 찼으면 리스트가 다 찼다는 메세지 출력
		if(studentCount == list.length) {
			System.out.println("리스트가 다 찼습니다.");
			return studentCount;
		}
		...
		list[studentCount] = tmp; // new Student(grade, classNum, num, name) 원래는 이 코드였지만 현재 tmp에 담겨 있으므로 tmp만 쓰면 됨.
	...
	public static void updateStudent(Student [] list, int studentCount, Scanner scan) {
		...
		if(index == -1) {
			System.err.println("일치하는 학생이 없습니다.");
			return;
		}
		
		// 있다면 국어, 영어, 수학 성적을 입력 받아 학생 성적을 수정
		System.out.print("국어 : ");
...
class Student {
	...
	public void print() {
		...
		System.out.println("국어 : " + kor + "점, 영어 : " + eng + "점, 수학 : " + math + "점 입니다.");
	}
